...
describe('check', () => {
  ...
  it('local process: static', async () => {
    ...
    const spansLength = sdk.log.ringBuffer
      .records
      .filter(({ log }) => log === 'zipkin-span')
      .length;
    expect(spansLength).toBe(2);
  });

  it('local process: promise ', async () => {
    await expect(sdk.zipkinTracer.local('promise int 1', () => Promise.delay(200, 1)))
      .resolves
      .toBe(1);
    await expect(sdk.zipkinTracer.local('promise int 2', () => Promise.delay(200, 2)))
      .resolves
      .toBe(2);
